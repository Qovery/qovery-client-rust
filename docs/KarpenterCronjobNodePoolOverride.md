...
**spot_enabled** | Option<**bool**> | Whether this node pool runs on spot instances. `null` or absent means the pool inherits the deprecated top-level `spot_enabled`: on write that value applies to this pool, on read only a deviating value is surfaced. Only meaningful while the cronjob node pool is enabled — the presence of `cronjob_override` is what enables it. | [optional]
