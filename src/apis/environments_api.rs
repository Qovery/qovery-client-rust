...
/// struct for typed errors of method [`get_project_environments_overview_deprecated`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetProjectEnvironmentsOverviewDeprecatedError {
    Status401(),
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
...
/// Returns a list of environments with their overview information including deployment status, service count, and cluster details.
pub async fn get_project_environments_overview_deprecated(
    configuration: &configuration::Configuration,
    project_id: &str,
) -> Result<
    models::EnvironmentOverviewResponseListDeprecated,
    Error<GetProjectEnvironmentsOverviewDeprecatedError>,
> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_project_id = project_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::EnvironmentOverviewResponseListDeprecated`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::EnvironmentOverviewResponseListDeprecated`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetProjectEnvironmentsOverviewDeprecatedError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
