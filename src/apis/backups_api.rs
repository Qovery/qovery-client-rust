...
pub async fn add_backup_database(configuration: &configuration::Configuration, database_id: &str, backup_request: Option<models::BackupRequest>) -> Result<models::Backup, Error<AddBackupDatabaseError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_database_id = database_id;
    let p_backup_request = backup_request;

    let uri_str = format!("{}/database/{databaseId}/backup", configuration.base_path, databaseId=crate::apis::urlencode(p_database_id));
...
pub async fn list_database_backup(configuration: &configuration::Configuration, database_id: &str, start_id: Option<&str>) -> Result<models::BackupPaginatedResponseList, Error<ListDatabaseBackupError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_database_id = database_id;
...
pub async fn remove_database_backup(configuration: &configuration::Configuration, database_id: &str, backup_id: &str) -> Result<(), Error<RemoveDatabaseBackupError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_database_id = database_id;
    let p_backup_id = backup_id;

    let uri_str = format!("{}/database/{databaseId}/backup/{backupId}", configuration.base_path, databaseId=crate::apis::urlencode(p_database_id), backupId=crate::apis::urlencode(p_backup_id));
