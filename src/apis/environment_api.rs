...
/// struct for typed errors of method [`list_environment_services_links`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListEnvironmentServicesLinksError {
    Status401(),
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
...
pub async fn list_environment_services_links(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListEnvironmentServicesLinksError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
