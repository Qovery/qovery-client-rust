...
/// struct for typed errors of method [`list_directories_from_git_repository`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListDirectoriesFromGitRepositoryError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_bitbucket_repository_branches(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetBitbucketRepositoryBranchesError> =
...
pub async fn list_directories_from_git_repository(
    configuration: &configuration::Configuration,
    organization_id: &str,
    application_git_repository_request: Option<models::ApplicationGitRepositoryRequest>,
) -> Result<
    models::ListDirectoriesFromGitRepository200Response,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListDirectoriesFromGitRepository200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListDirectoriesFromGitRepository200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListDirectoriesFromGitRepositoryError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
