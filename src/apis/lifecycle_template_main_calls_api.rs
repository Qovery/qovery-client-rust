...
pub async fn get_environment_lifecycle_template(configuration: &configuration::Configuration, environment_id: &str, lifecycle_template_id: &str) -> Result<models::LifecycleTemplateResponse, Error<GetEnvironmentLifecycleTemplateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_environment_id = environment_id;
    let p_lifecycle_template_id = lifecycle_template_id;

    let uri_str = format!("{}/environment/{environmentId}/lifecycleTemplate/{lifecycleTemplateId}", configuration.base_path, environmentId=crate::apis::urlencode(p_environment_id), lifecycleTemplateId=crate::apis::urlencode(p_lifecycle_template_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetEnvironmentLifecycleTemplateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn list_environment_lifecycle_templates(configuration: &configuration::Configuration, environment_id: &str) -> Result<models::LifecycleTemplateListResponse, Error<ListEnvironmentLifecycleTemplatesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_environment_id = environment_id;

    let uri_str = format!("{}/environment/{environmentId}/lifecycleTemplate", configuration.base_path, environmentId=crate::apis::urlencode(p_environment_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListEnvironmentLifecycleTemplatesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
