...
/// struct for typed errors of method [`list_cluster_deployment_history_v2`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListClusterDeploymentHistoryV2Error {
    Status401(),
    Status403(),
    Status404(),
...
/// struct for typed errors of method [`list_cluster_deployment_logs`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListClusterDeploymentLogsError {
    Status401(),
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
...
pub async fn list_cluster_deployment_history_v2(
    ...
    models::ClusterDeploymentHistoryPaginatedResponseListV2,
    Error<ListClusterDeploymentHistoryV2Error>,
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ClusterDeploymentHistoryPaginatedResponseListV2`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ClusterDeploymentHistoryPaginatedResponseListV2`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListClusterDeploymentHistoryV2Error> =
...
pub async fn list_cluster_deployment_logs(
    configuration: &configuration::Configuration,
    organization_id: &str,
    cluster_id: &str,
    deployment_id: &str,
) -> Result<models::ClusterLogsResponseList, Error<ListClusterDeploymentLogsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_organization_id = organization_id;
    let p_path_cluster_id = cluster_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListClusterDeploymentLogsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
