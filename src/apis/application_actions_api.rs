...
/// struct for typed errors of method [`uninstall_application`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UninstallApplicationError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn reboot_application(
    configuration: &configuration::Configuration,
    application_id: &str,
...
pub async fn uninstall_application(
...
) -> Result<serde_json::Value, Error<UninstallApplicationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_application_id = application_id;
    let p_body = body;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UninstallApplicationError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
