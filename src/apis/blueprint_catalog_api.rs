...
pub async fn get_blueprint_catalog_service_manifest(
    ...
    service_version: &str,
    environment_id: &str,
) -> Result<models::BlueprintManifestResponse, Error<GetBlueprintCatalogServiceManifestError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::BlueprintManifestResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::BlueprintManifestResponse`")))),
        }
    } else {
        let content = resp.text().await?;
