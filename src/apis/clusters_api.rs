...
/// struct for typed errors of method [`get_eks_anywhere_cluster_jwt`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetEksAnywhereClusterJwtError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
/// struct for typed errors of method [`get_environments_by_cluster_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetEnvironmentsByClusterIdError {
    Status401(),
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
...
pub async fn get_eks_anywhere_cluster_jwt(
    configuration: &configuration::Configuration,
    organization_id: &str,
    cluster_id: &str,
) -> Result<models::EksAnywhereClusterJwtResponse, Error<GetEksAnywhereClusterJwtError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_organization_id = organization_id;
    let p_path_cluster_id = cluster_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::EksAnywhereClusterJwtResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::EksAnywhereClusterJwtResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetEksAnywhereClusterJwtError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_environments_by_cluster_id(
    configuration: &configuration::Configuration,
    cluster_id: &str,
) -> Result<models::ClusterEnvironmentResponseList, Error<GetEnvironmentsByClusterIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cluster_id = cluster_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ClusterEnvironmentResponseList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ClusterEnvironmentResponseList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetEnvironmentsByClusterIdError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
