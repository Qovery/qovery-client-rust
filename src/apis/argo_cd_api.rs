...
/// struct for typed errors of method [`get_argo_cd_associated_services`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetArgoCdAssociatedServicesError {
    Status401(),
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
...
pub async fn get_argo_cd_associated_services(
    configuration: &configuration::Configuration,
    cluster_id: &str,
) -> Result<models::ArgocdAssociatedServicesResponseList, Error<GetArgoCdAssociatedServicesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cluster_id = cluster_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ArgocdAssociatedServicesResponseList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ArgocdAssociatedServicesResponseList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetArgoCdAssociatedServicesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
