...
pub async fn list_application_deployment_history(configuration: &configuration::Configuration, application_id: &str, start_id: Option<&str>) -> Result<models::DeploymentHistoryPaginatedResponseList, Error<ListApplicationDeploymentHistoryError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_application_id = application_id;
    let p_start_id = start_id;

    let uri_str = format!("{}/application/{applicationId}/deploymentHistory", configuration.base_path, applicationId=crate::apis::urlencode(p_application_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListApplicationDeploymentHistoryError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// By default it returns the 20 last results. The response is paginated. In order to request the next page, you can use the startId query parameter. You can also filter by status (FAILED or SUCCESS), and git_commit_id
pub async fn list_application_deployment_history_v2(configuration: &configuration::Configuration, application_id: &str, start_id: Option<&str>) -> Result<models::DeploymentHistoryServicePaginatedResponseListV2, Error<ListApplicationDeploymentHistoryV2Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_application_id = application_id;
    let p_start_id = start_id;

    let uri_str = format!("{}/application/{applicationId}/deploymentHistoryV2", configuration.base_path, applicationId=crate::apis::urlencode(p_application_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListApplicationDeploymentHistoryV2Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
